...
3. 代理转发
   - 访问接口 A 的请求转发到接口 B。
   - 因为项目是专属化部署（就是布置到客户自己的服务器上），那么一些服务的地址就不能使用绝对路径，只能使用相对路径。而在本地化开发的过程中，线上服务使用的是相对路径，请求时，其域名拼接的是本地的 host，会出现访问不到的情况，因此，这种情况下，需要使用 BFF 中间层做代理，在中间层配置好线上的域名，将本地的请求转发到线上。
...
1. 设置环境信息中间件
   - 在进行代理转发的时候，需要区分是本地环境还是生产环境。之前的做法是在每个路由中都做处理，进行区分，随着路由的增多，这样的区分方法会带来大量的重复代码，且效率比较低。因此将这部分功能抽出来，单独做一个中间件，在这个中间件内部，根据 cookies、url 中的查询参数、基础配置项的信息判断当前是生产环境还是本地环境，然后将环境信息挂载到 ctx 对象上。根据不同的环境请求不同的静态资源信息。

2. 生成模板字符串的中间件
   - 在 ctx 上挂载一个 render 方法，这个方法根据输入的数据生成一份 html 模板。在需要返回 html 的地方，直接调用 render 方法即可。

3. 代理中间件
   - 根据 ctx.path 或者一些配置信息，判断哪些请求需要直接代理转发到线上。
   

## 12. 应用构建是做什么的，包括应用引擎、流水线等

1. 应用构建提供了一套完整的开发部署工具，包括服务器（资源）、持续集成（流水线）、权限分配、前后端开发脚手架等。

2. 新建应用引擎后，就可以这个引擎为基础，开发自己的应用了。
...
3. 原生的 input 元素样式不美观，因此将其隐藏。用户可以自定义表示上传的元素。可以是按钮，也可以是图片。那么如何触发上传操作呢，我们这里使用了事件委托机制，利用事件冒泡，在其父元素上绑定点击事件处理函数，当自定义元素被点击后，点击事件向上传递，在父元素上被捕捉到，此时表示有上传操作，那么在点击事件的处理函数中，我们再手动触发 input 元素的 click 事件，就间接模拟了用户的点击 input 元素的操作。

### JavaScript 事件机制 - 事件冒泡、事件捕获、事件委托
...
### 跨域的相关内容 - JSONP、CORS、PostMessage
...
### CSS 3 新属性 - transition、transform、animation、flex

## 16. 开发模态框组件遇到什么问题

1. 如何将模态框插入到应用的组件之外
   - ReactDOM.createPortal
   - 这个函数的主要作用是实现将子节点渲染到父组件DOM层次结构之外的DOM节点。
   - 使用 createPortal() 将模态框挂载到 body 元素下层。

2. 遮罩层的实现
   - 主要是 CSS 样式的设置。设置其 position 属性为 fixed，然后设置其 top、right、bottom 和 left 均为 0，这样可以使得遮罩层铺满可视区域。最后设置遮罩层的背景颜色和 z-index 属性，保证遮罩层在其他元素的上面。
...
3. JavaScript 模拟 hover 操作
   ...
   - css 提供了 hover 这个伪类来实现悬浮操作，当某个元素的 hover 动作发生时，只能给这个元素及其子元素或者是其后面的相邻兄弟元素设置样式，不能给前面的兄弟元素设置样式。而一个需求就是下面的元素有 hover 操作时，隐藏上面的元素。因此单纯使用 hover 伪类不能实现这个需求，只能借助 JavaScript 模拟 hover 操作。

4. 开发附件上传组件过程中，如何精确限制文件类型
   - input 元素有一个 accept 属性，这个属性用来限制上传文件的类型。
   - 但是 accept 属性是通过文件后缀来识别文件的，换句话说，就是被限制上传的文件，改一下后缀，就会被识别为允许的问题，从而实现上传。
   - 因此需要精确识别文件类型。所以精确识别文件类型是通过幻数（magic number）实现的，所谓的幻数，就是指定类型的二进制文件中的前几个字节的内容都是固定的，计算机是通过识别这几个字节来判断文件类型的。
   - 使用 FileReader api 将 File 对象转换为 ArrayBuffer 示例，然后根据识别的文件类型从 ArrayBuffer 实例中，取出前几位字节，进行判断。

### JavaScript 中的异步 - 事件循环、宏任务、微任务

### Promise - Promise 的基本应用、状态变化、手写 Promise

### async / await - 基本用法、原理

### 跨域通信 - http / https / http 缓存
...
### 函数式编程 - once 函数、柯里化、map、reduce 的应用
