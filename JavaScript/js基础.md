...
1. 一般情况下，使用 `var` 关键字定义变量，如 `var a = 123;`，js 引擎在编译阶段，会将声明提升到它所在作用域的顶端去执行，到我们代码所在的位置来赋值。简单的说，就是先提升声明到顶端，再赋值。
...
2. 示例：
   ...
   ```js
      ...
      a = 123;
   ```
3. JavaScript 中函数的声明也会被提升。具名函数的声明有两种方式：函数声明式和函数字面量式，如下所示：
   ```js
      //函数声明式
      function bar () {}
      //函数字面量式
...
4. 函数字面量式的声明和变量提升的结果是一样的，函数只是一个具体的值。但是函数声明式的提升现象和变量提升略有不同：
   ```js
      console.log(bar);
   ...
   ```
   输出如下图所示：
   ![img.png](img/variable-ascension.png)
   通过函数声明定义的函数，其提升是整个代码块提升到它所在的作用域的顶端。

5. **函数提升的优先级高于变量提升**。如下所示：
   ```js
     // 1
      bar();
   ...
   ```
   使用 var 关键字声明了变量 bar，同时使用 function 声明了函数 bar，二者同样会被提升到当前作用域的顶部，但是函数提升优先于变量提升，所以 bar 就是函数声明的表达式，然后我们再直接调用，就输出 1。
...
6. 再看一个示例：
   ...
   2. 声明函数 bar。
   3. 调用函数 bar。
   4. 在 bar 的内部，同样有声明语句：`var foo = 10`，那么会这个声明提升到 bar 作用域的顶部。等同于：
      ```js
         function bar() {
             var foo;
             if (!foo) {
      ...
      ```
   5. 在 bar 的内部，执行到 if 这一句，发现要找变量 foo，那么优先在当前 bar 的作用域内寻找，发现声明了 foo 这个变量，那么就会使用 bar 作用域内的 foo，此时只是声明了 foo，因此其值是 undefined。所以，if 语句这里判断为 true，进入 if 的代码块中，给 foo 赋值为 10。
   6. 最后执行 `console.log(foo);`，此时 foo 已经被赋值为 10 了，因此输出 10.
