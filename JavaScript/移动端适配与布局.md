...
6. 不管是移动优先还是PC优先，都是依据当随着屏幕宽度增大或减小的时候，后面的样式会覆盖前面的样式。因此，移动端优先首先使用的是`min-width`，PC 端优先使用的 `max-width`。
...
1. css3中引入了一个新的单位 vw/vh，与视图窗口有关，vw表示相对于视图窗口的宽度，vh表示相对于视图窗口高度。
...
所有设置的固定值都用 rem 做单位（首先在 HTML 总设置一个基准值：px 和 rem 的对应比例，然后在效果图上获取 px值，布局的时候转化为rem值）。
...
1. 常见的设计图宽度是 375px，当然也可以是其他的宽度，比如 720 像素的：
   - 375 iPhone7
   - 750 二倍图
   - 320 iPhone5
   - 640 二倍图

2. 为什么给的是 375？因为这个是 iPhone7 的宽度，
